...
## Serving & deployment stack (decided — keep unless the GPU blocker resolves)
- **Inference engine = Ollama, not vLLM.** vLLM's wins (PagedAttention, continuous batching) are GPU-shaped and largely evaporate on CPU. Its CPU backend effectively wants **AVX512** (our AVX support is *uncertain* — VM masked as Core 2 Duo T7700, §"VM specs") and usually needs a from-source/custom Docker build, with weak quantization support — exactly backwards, since **quantized GGUF (Q4) is the #1 CPU throughput lever** and is Ollama's default. Ollama is also one binary on the OpenAI-compatible `/v1` the app already targets. **Conditional flip:** *if* the client provisions a real GPU (blocker #1), re-evaluate — vLLM's batching then becomes a genuine win for 10 concurrent users.
- **Deployment depends on phase:**
  - **POC phase (current — internet-connected, not yet official, heavy front/back iteration):** Coolify + Docker for the **Next.js app** is reasonable — push-to-deploy, auto-SSL, easy rollback are exactly what a fast-iterating "do they want it?" POC needs, and the gov-hardening/egress objections don't apply yet. **Caveats:** run **Ollama standalone** (its own container or native, reachable at `127.0.0.1:11434`) — not inside Coolify's app pipeline — to keep a clean GPU-passthrough path; and put **Postgres data + Ollama models on persistent named volumes** so an app redeploy never nukes the **~10h full embed** or forces a model re-pull. Verify the volume survives a redeploy *before* running the long embed.
  - **Production / official phase:** revert to **native + `systemd`** (the supported path — [vm-deployment.md](vm-deployment.md) §1, "preferred over Docker on a locked-down gov box"; `deploy/` units cover app + sync timer). At go-live the gov objections return: Coolify mandates Docker everywhere, pulls images over the network (collides with egress blocker #2), and adds an internet-facing control-plane dashboard = new attack surface to harden/audit. Treat Coolify as POC scaffolding, not the thing handed to their security team.
