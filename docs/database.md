...
| `chatbot_settings` | **single row (`id=1`, CHECK constraint)** — bot_name, welcome_heading, welcome_subtitle, starter_prompts (jsonb array), refusal_message, maintenance_enabled, maintenance_message, avatar_data (bytea) + avatar_mime + avatar_updated_at, updated_at. Admin-editable chatbot branding/copy + maintenance + uploaded avatar. Read by `getChatbotSettings()` (`src/lib/chatbotSettings.ts`); edited via `/admin` → **Chatbot** tab → `/api/admin/chatbot-settings` (text) + `/api/admin/chatbot-settings/avatar` (upload); avatar served by public `/api/chatbot/avatar`. Defaults mirror `src/lib/chatbotDefaults.ts`. **No RLS/GRANTs** (app connects as owner, like `users`). **Existing DBs: add non-destructively with `npx tsx scripts/migrate-chatbot-settings.ts`** — `setup-db.ts` only creates it on a full reset. |
