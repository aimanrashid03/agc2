        ...
        for (const dir of directories) {
            ...
            for (const c of casesData) {
                ...
                const alreadyProcessed = await pool.query(
                    'SELECT 1 FROM case_embeddings WHERE case_id = $1 LIMIT 1',
                    [caseId]
                );

                if (alreadyProcessed.rows.length > 0) {
                    console.log(`  Skipping case ${c.LKK_INFOID}: already processed.`);
                    continue;
                }

                // 3. Construct text content
                const textParts = [
                ...
                for (const doc of docs) {
                    ...
                    try {
                        ...
                        console.error(`  Failed embedding for case ${c.LKK_INFOID}:`, err);
                    }
                }
